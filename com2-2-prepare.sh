...
# Virtualization type modification in nova-compute.conf
if [ $(egrep -c '(vmx|svm)' /proc/cpuinfo) == 0 ]; then
filenovacompute=/etc/nova/nova-compute.conf
test -f $filenovacompute.orig || cp $filenovacompute $filenovacompute.orig

cat << EOF > $filenovacompute
[DEFAULT]
...
# Update config file /etc/nova/nova.conf
cat << EOF > $filenova
[DEFAULT]
verbose = True
...
sleep 5
apt-get install neutron-common neutron-plugin-ml2 neutron-plugin-openvswitch-agent openvswitch-datapath-$
...
[service_providers]
service_provider=LOADBALANCER:Haproxy:neutron.services.loadbalancer.drivers.haproxy.plugin_driver.Haprox$
