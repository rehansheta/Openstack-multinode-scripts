...
# Update config file /etc/neutron/l3_agent.ini
cat << EOF >> $netl3agent
[DEFAULT]
verbose = True
...
# Update config file /etc/neutron/dhcp_agent.ini
cat << EOF >> $netdhcp
[DEFAULT]
interface_driver = neutron.agent.linux.interface.OVSInterfaceDriver
...
# Update config file /etc/neutron/metadata_agent.ini
cat << EOF >> $netmetadata
[DEFAULT]
verbose = True
